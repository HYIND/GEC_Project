...
int lcd_draw_jpg(unsigned int x, unsigned int y, const char *pjpg_path, char *pjpg_buf, unsigned int jpg_buf_size, unsigned int jpg_half)
{
	...
	if (pjpg_path != NULL) //显示一张jpg图片
	{
		...
		/* 为jpg文件申请内存空间 */
		pjpg = malloc(jpg_size); //相当于一个数组

		/* 读取jpg文件所有内容到内存 */
		read(jpg_fd, pjpg, jpg_size); //把图片的的值读取出来存放在以pjpg为首的内存地址
	}
	else //摄像头数据
	{
	...
	else
	{
		...
		while (cinfo.output_scanline < cinfo.output_height)
		{
			...
			{
				/* 获取rgb值 */
				color = *(pcolor_buf + 2);				//第三个字节
				color = color | *(pcolor_buf + 1) << 8; //第二个字节
				color = color | *(pcolor_buf) << 16;
